...
#%%
# GLOBAL SETTINGS (also used in notebooks/dev.py
ksplit: int = 3
...
batch = 0.9     # if batch=-1 it uses 60% of GPU memory, of set to float <1: uses % of GPU memory, if int, uses batch size
epochs = 10
imgsz = 640
...
print(f'Training completed in {total_time_spent:.2f} seconds, which is equivalent to {total_time_spent / 60:.2f} minutes')
for i, fold_time in enumerate(fold_times, start=1):
    print(f'Time spent on fold {i}: {fold_time:.2f} seconds, which is equivalent to {fold_time / 60:.2f} minutes')